            ...
            { 
                venues.filter(venue => view === 'venues' || (venue.visited && view === 'visited') || (!venue.visited && view === 'mustvisit')).map(venue => {
                    ...
                    return (
                        <>
                        <li key={ venue.id }>
                            <h3> { venue.name } <button onClick={()=>deleteVenue(venue)}>DELETE</button></h3>
                            ...
                            <div>
                                (Neighborhood: {!venue.neighborhood.name ? '' : venue.neighborhood.name})
                            </div>
                        </li>
                        </>
                    );
...
const mapDispatchToProps = (dispatch) => {
    return {
        deleteVenue: venue => dispatch(deleteVenue(venue)),
